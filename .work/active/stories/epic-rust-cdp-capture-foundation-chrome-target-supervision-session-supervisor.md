...
- Root cause: `ManagedChromeProcess::terminate` returned as soon as the direct Chrome child was reaped, even when Chrome helpers still occupied its isolated process group; the same gap existed on natural-exit and drop paths.
- Fix: capture and validate the isolated PGID at spawn, use `getpgid` while the leader is alive and current group membership after it exits, wait boundedly for graceful group disappearance, force-kill remaining owned members, reap the direct child, and require a final no-members check before successful async termination. Drop/force cleanup follows the same ownership guard and boundedly confirms group disappearance.
- Regression coverage: `tests/process_ownership.rs` proves both `terminate` and drop remove a group descendant whose leader exits on TERM while the descendant ignores TERM. Real Chrome coverage asserts no command line references each unique profile root after stop and before RAII root cleanup; the guard preserves roots when references remain.
- Execution capability: inline implementation, as explicitly requested; review weight: standard; story intentionally remains at `stage: review` for handoff.
- Files changed: `src/launcher/process.rs`, `tests/process_ownership.rs`, `tests/support/chrome.rs`, and `tests/chrome_session_real.rs`.
- Verification: workspace default and no-default test suites (101 tests each), denied-warning workspace clippy, and three opt-in real-Chrome suite runs after the fix; all passed. Final checks found zero `krometrail-real-*` directories and zero matching live process command lines.
