...
- Exactness evidence: `cargo test --workspace --all-targets --locked` passed with 709 tests and 4 ignored. Baseline/context service fixtures matched artifact IDs, cache dispositions, manifests, encoded bytes, hashes, source ordering, orientation roles, timestamps, gaps, and masks. `cargo clippy --workspace --all-targets --locked -- -D warnings` and formatting passed.
