...
- [ ] Scripted CDP observes the requested `everyNthFrame` for both JPEG and PNG starts; production
      code has no hardcoded `everyNthFrame: 1` parameter.
- [ ] Initial and dynamic target starts use the same session value, while existing subscribe-before-
      start and ack-first ordering remain intact.
- [ ] A scripted physical transport disconnect/reconnect sends the same requested value on the new
      connection, preserves target identity and attachment-generation rules, and does not emit a
      stride-derived capture gap.
- [ ] Status and capture-state event values equal the request value; observed cadence, queue drops,
      persistence failures, visibility gaps, ordinals, and gap reasons remain independent.
- [ ] Tests use existing fake/scripted transport seams only. No real Chrome, model, network, or
