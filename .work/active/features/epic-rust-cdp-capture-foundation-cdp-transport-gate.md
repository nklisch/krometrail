...
- **Candidate order:** Pin the published `cdpkit` crate exactly to `=0.4.0` and test it unchanged first. `chromey` and owned-transport work are late-bound only after a demonstrated cdpkit failure; no speculative fallback story is created during design.
