...
- During the original aggregate verification, concurrent `truthful-screencast-geometry` CDP edits produced four `await_holding_lock` findings and one index-out-of-bounds test failure. Those dependency fixes landed separately; no CDP file is part of this feature's commits.
...
- Captured bounds now reach `range_not_found` only when an empty interval has no intersecting durable eviction. A regression requests a fully evicted interval while later frames remain retained and proves the stable eviction response keeps `retry: never` with no recovery.
