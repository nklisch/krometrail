...
- **Presentation timebase**: plan boundaries are deterministically rounded from nanoseconds to a one-microsecond FFconcat/filter/MP4 timebase by rounding cumulative endpoints, never each duration independently. The adapter accepts the plan only when every source PTS plus the terminal sentinel at duration minus one tick is strictly increasing; collapsed sub-tick or dense boundaries fail explicitly as unrepresentable timing before launch. The generated PTS filter assigns those validated boundaries explicitly, and the final one-tick sample closes the track exactly. The v1 argument-policy identity covers this representability rule; source/session provenance remains the exact committed plan.
...
- **Runtime identity pinning**: qualification stores the canonical executable and a platform file stamp plus executable digest. Each encode rechecks the canonical path and file stamp before staging and launch; a final spawn failure is followed by another identity check so ordinary replacement or disappearance maps to `video_encoder_unavailable` until restart. Defending against a malicious same-user replacement preserving metadata during the final launch race is outside the local-user trust boundary.
...
- Spawn with null stdin, piped and bounded stdout/stderr, `kill_on_drop(true)`, a scrubbed environment, and no inherited handles beyond required pipes. Never use `sh`, `cmd.exe`, `powershell`, `Command::arg` with a joined command line, or an external kill helper.
- On the supported Linux/macOS runtime create an isolated process group and apply `RLIMIT_CPU` from the bounded remaining deadline before exec. Other platforms, including Windows, return an explicit unsupported-platform qualification outcome; no unproven privacy or process-tree guarantee is advertised.
...
- [x] Tests exercise the process guard on supported Unix platforms; unsupported platforms fail qualification explicitly instead of silently degrading.
...
- Require `ftyp`, one `moov`, media data, exactly one video track, `vide` handler, `avc1` or `avc3` sample entry, matching canvas dimensions, the fixed 1 MHz track timebase, a duration within one microsecond tick of the prepared presentation, bounded `stsz`/`stts` sample counts, decode durations exactly matching the quantized plan, zero composition offsets when `ctts` is present, and no `soun` handler. Magic bytes or an encoder exit code alone do not qualify output.
...
- PATH/default discovery is internal—no `which` subprocess—and bounds the total probe count to 16 canonical, deduplicated candidates named exactly `ffmpeg`. Platform defaults are fixed Linux/macOS policy data, not recursive filesystem searches.
...
- Reuse Tokio, SHA-256, tracing, libc, and the repository's hermetic-boundary/opt-in-qualification testing pattern. Add only `tempfile`; unsupported platforms remain an explicit unavailable outcome.
...
- **FFconcat and MP4 cannot represent arbitrary nanoseconds exactly.** Mitigation: cumulative endpoint rounding to a declared microsecond timebase is deterministic and included in the argument-policy identity; any rounded source or terminal PTS collision fails explicitly as unrepresentable timing. The exact source plan remains manifest authority.
...
- **Unsupported platforms cannot prove the Unix cleanup/privacy boundary.** Qualification returns `UnsupportedPlatform` before discovery or process launch. The capability remains optional, so explicit unavailability is preferable to advertising a degraded encoder.
...
- The process boundary uses no shell, wrapper, downloader, network client, native FFmpeg binding, or managed binary. Supported Linux/macOS runtimes use Unix process groups plus `RLIMIT_CPU`; other platforms fail qualification explicitly. Default tests use a compiled Rust fixture, and the real interoperability lane is feature-gated, ignored, and requires an explicit executable.
...
- No adjacent issue was discovered or parked. This macOS implementation run exercised the supported Unix process-group path and live FFmpeg 8.0.1 path.
...
- Added one `OperationControl` carrying the core cancellation signal and exact deadline through bounded discovery, executable metadata/hash work, private staging, post-exit reads, MP4 parsing/hashing, and final core clip construction. Filesystem and parser work now runs through `spawn_blocking`; cancellation/deadline tests cover in-flight blocking work and validation.
- Made microsecond timing total by either producing strictly increasing source/terminal PTS or returning the explicit internal `UnrepresentableTiming` failure, which maps to the stable public encoding failure. Dense, collapsed sub-tick, one-tick terminal, cumulative-rounding, and overflow boundaries are covered.
- Extended the bounded MP4 proof to require `stsd`, `stsz`, and `stts`, cap total samples at the core plan ceiling plus the terminal sentinel, compare every decode duration with the quantized plan, and reject non-zero `ctts` offsets. Mutation tests cover count, timeline, composition-offset, and cancellation failures.
- Regenerated the retained final-policy fixture through the ignored live lane. It is a 1,508-byte, three-sample, 350 ms H.264 MP4 with SHA-256 `8b3905f2acd80fc1f4c2a476e8339ca0c17d79c0efa668ba0f6894f6fad2c762`; neighboring provenance records the terminal-sentinel timing.
- Removed the Windows Job Object dependency and implementation. Qualification is explicitly unavailable outside the supported Linux/macOS runtime, while supported Unix process ownership remains fail-closed.
- The policy test now compares the complete ordered argv vector. A spawn failure followed by missing/drifted executable identity maps to `ChangedCandidate`, and the encoder boundary maps that to `video_encoder_unavailable`.
- Focused correction gate: `cargo test -p krometrail-ffmpeg` passed 25 unit tests, 11 adapter tests, and doc tests; `cargo clippy -p krometrail-ffmpeg --all-targets -- -D warnings` passed.
- Live correction lane: `KROMETRAIL_FFMPEG_PATH=/opt/homebrew/bin/ffmpeg KROMETRAIL_FFMPEG_FIXTURE_OUTPUT=... cargo test -p krometrail-ffmpeg --features qualification-support --test ffmpeg_live -- --ignored --nocapture` qualified FFmpeg 8.0.1/libx264 and produced the retained 1,508-byte fixture.
- Authoritative combined serial gate on the coherent FFmpeg + retained-generation tree passed `cargo fmt --all -- --check`, `cargo check --workspace --all-targets --locked`, `cargo test --workspace --all-targets --locked`, and `cargo clippy --workspace --all-targets --locked -- -D warnings`.
- No second independent review was run; the item returns to `stage: review` for correction verification as required by the standard review policy.
