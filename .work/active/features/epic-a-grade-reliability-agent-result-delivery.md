...
## Installed usage after update/restart

After the user updated Pi Extensions and restarted, ordinary live `start_browser`/`list_pages` calls still delivered only summary text. Read-only inspection of the installed `@nklisch/pi-mcp-adapter` at `~/.pi/agent/npm/node_modules/@nklisch/pi-mcp-adapter` found version `2.21.0-nklisch.2`; both `tool-registrar.ts` and `dist/tool-registrar.js` still contain `if (blocks.length > 0) return blocks` and lack the structured-append label. The corrected repository source therefore has not reached this installation. No package was altered or installed by this exercise.

Scripted extraction exposed canonical page inventories and status, distinguishing summary suppression from a separate live target-attachment failure. The full bounded example.com/example.org reproduction is in the existing page-selection-recovery item. Actual model-visible success and screenshot/interaction qualification remain blocked, not passed. Both temporary test browsers were stopped.

## Authorized publication and local installation

The user explicitly authorized publishing the relevant releases and installing them locally, and offered to log in if required. This supersedes the earlier source-only boundary: Krometrail and its plugin projections, independent temporal-vision 0.2.0, the fixed Pi adapter and required consumer/bundle bumps, and the external nklisch/skills Krometrail catalog entries are in scope. Unrelated Ollama work remains excluded. Actual registry/release receipts, installed bytes, and a fresh-host workflow remain required before claiming delivery.

Release preparation found the known false Rust 1.85 declaration. Explicit Rust 1.88.0 locked workspace/all-targets check and tests passed at `90085cba5e440a6227013cd6adf574fdf59969ac` in `/tmp/krometrail-release-msrv.log`; the owned compiler-specific target was removed. The minimum-version owner is correcting the declaration and explicit CI selection before release. Stable formatting/Clippy remain a separate gate. GitHub authentication succeeds; the crates-io environment has no required-reviewer protection, and the prior temporal-vision publication workflow succeeded. These are readiness observations, not new publication receipts.
