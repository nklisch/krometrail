...
impl RecordingCatalog for SqliteIndex {
    ...
    fn note_terminal_session(&self, session: RecordingSession) {
        if session.lifecycle() != krometrail_core::SessionLifecycle::Ended {
            return;
        }
        if let Ok(mut overrides) = self.terminal_session_overrides.lock() {
            overrides.insert(session.id(), session);
        }
    }
