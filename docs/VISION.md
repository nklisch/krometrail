...
The live observation loop supports normal browser use. Responses are concise and action-centric by default; agents deliberately expand them for broader page context or full evidence. Explicit visual operations include one bounded useful image by default, while routine actions remain pixel-light and can request pixels when needed. Canonical resources and explicit inspection preserve drill-down authority without forcing routine actions to carry complete trees, diagnostics, or provenance. Semantic targeting and intention-revealing viewport controls reduce orchestration overhead without hiding target, document, frame, or effective-layout boundaries. Interaction results pair successful dispatch with bounded observed postconditions so an agent can tell whether navigation, a control-state change, or a side-channel outcome actually occurred without a separate investigation. The temporal layer is an additional queryable history, not a replacement for current-state vision.
...
The agent can expand a region, retrieve additional source frames, or reuse a compact resolved-range handle without loading the entire recording or repeating its full provenance structure in context.
