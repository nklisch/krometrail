...
The bound: the combined footprint of live instances settles at the configured total, and an instance already over its share trims on its own next append. It is not an instantaneous guarantee. Consulting the shared ledger takes a lock, so it is kept off the per-frame path; an instance reuses its share until either two seconds pass or it has grown by 1/32 of the total, whichever comes first. Growth inside that window is spent against a figure its peers have not seen yet, so the instantaneous combined footprint may exceed the total by up to 1/32 of the total per live instance before the next accounting transaction pulls it back.
