...
The second is the equal-share floor. An instance that grew while it was alone keeps what it holds when a peer starts, because isolation forbids either one from reclaiming the other's data, and the new peer is still guaranteed its equal share. With `N` live instances that permits at most a further `(N-1)/N` of the total. This excess is transient: the over-sized instance trims on its own next append or enforcement pass, its evidence ages out, and when it exits its bytes stop counting immediately.

The precise instantaneous bound is therefore `total + (N-1)/N × total + N × total/32` for `N` live instances, converging on the configured total as each instance's next accounting pass runs.
