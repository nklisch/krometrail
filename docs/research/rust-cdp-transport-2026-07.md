...
Evidence was frozen on 2026-07-12 from crates.io metadata, published/repository source at `cdpkit` 0.4.0 (`15dd5e6d`) and `chromey` 2.52.0 (`6eeca5e8`), their CI/tests, their public GitHub issues, and the final schema-v2 qualification. The checked-in reports and generated decision are now historical because they predate the trace-reconstructability contract; their bytes remain unchanged and no fresh browser evidence is generated by that repair. A future decisive report must preserve bounded sanitized canonical fixture, wire, and runtime material and recompute all summaries from it. The revised contract uses observed `capture_elapsed_seconds` and `handoff_elapsed_seconds` measurements and measures acknowledgement from returned frame to ack completion only. No production lifecycle or capture implementation is implied.
...
The retained version-1 reports and prior-v2 reports/decision are historical inputs only. The previously checked-in final reports are also obsolete as decisive inputs after the trace-reconstructability repair: they contain only candidate-contract summaries and digests, not the canonical material needed to recompute them. Their bytes remain unchanged; no replacement browser run is claimed here. The identity and measurements below describe that historical run and do not satisfy the repaired decisive contract.
