...
This directory contains the **prior schema-v2 qualification reports and generated decision**. The trace-reconstructability contract superseded these report bytes: they intentionally remain unmodified and are no longer decisive inputs because they do not carry the canonical material required for independent recomputation. No fresh browser evidence is generated by that contract repair. The schema remains the Rust-generated v2 contract at [`schema.json`](./schema.json); future decisive reports must include `candidate_contract.trace`.
...
The checked-in Linux/macOS reports and decision below document the earlier qualification run for historical audit only. Validation now rejects a decisive report unless its bounded sanitized candidate trace contains the ordered fixture projection, wire observations, and runtime assertions and every duplicated digest/count/result is recomputed from that material. The historical JSON files and historical provenance are deliberately untouched.
