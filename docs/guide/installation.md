...
verify it against `checksums.txt`, require the temporary binary's exact
`krometrail <selected-semver>` `--version` output, and replace an existing
installation only after all checks pass.
