...
The current schema-v2 Linux/macOS reports and generated decision in `docs/evidence/cdp-transport/v2/` are the final qualification record from exact revision `07b0990c0d9e4fea9057fcab5c35e56691ff69eb`; report digests are Linux `a7195eda1667e613b1b3f857fd56cc60153500544493a86afac8448706d20270` and macOS `46901e41bb2a4bb674d76d9dce41fc4200032280cd9720daaaad965ee89d257b` from hosted run `29207244853`, with decision digest `91f9032315dd3501068e1dd692b12fbda7ce0d7a57c9b5a49444db73c2a5c015`. Retained v1 and prior-v2 reports/decision are historical only. The reports share source attestation `sha256:b4147b12577e980123bfb711d314dd17f22b0639303956e97441af74a8b297b0`, the committed browser fixture and candidate-contract trace digests, and 942 trace observations. The contract names observed `capture_elapsed_seconds` and `handoff_elapsed_seconds` measurements, keeps the configured 120-second global hard stop authoritative when frame minima are unmet, and measures acknowledgement only from returned frame to ack completion. Post-receive ack p99/max are Linux `0.3979589999999999/2.785427 ms` and macOS `1.062666/7.058083 ms`; handoff follows acknowledgement. The adapter must preserve cdpkit's named-event-params-only escape hatch: it is not wildcard or full-envelope receive, and its subscriber queue depth is not inspectable. Krometrail therefore owns prompt acknowledgement before bounded handoff, backpressure and capture-gap policy, reconnect/session restoration, cancellation, and flush behavior. A cdpkit routing, decoder, lifecycle patch, or fork would invalidate the selection and trigger the documented fallback rules. The final decision selects exact cdpkit 0.4.0 without a waiver.
...
- Final schema-v2 evidence selected exact cdpkit 0.4.0 behind a replaceable adapter from revision `07b0990c0d9e4fea9057fcab5c35e56691ff69eb`; its named-event-params and unbounded-subscriber limitations remain explicit, and the adapter must pass the required-domain compatibility probe at runtime.
