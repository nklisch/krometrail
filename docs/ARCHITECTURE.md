...
Historical pre-remediation evidence identified exact published `cdpkit` 0.4.0 behind the replaceable `krometrail-cdp::transport` boundary, but that selection is suspended until strict schema-v2 Linux/macOS reports requalify from one immutable gate implementation revision, configuration, and fixture. Version 2 preserves platform-labelled gate results and candidate-contract trace/hash/results, and requires canonical RSS sample/cadence/warmup plus observed lifecycle measurements. Any eventual adapter must preserve cdpkit's named-event-params-only escape hatch: it is not wildcard or full-envelope receive, and its subscriber queue depth is not inspectable. Krometrail therefore owns prompt acknowledgement before bounded handoff, backpressure and capture-gap policy, reconnect/session restoration, cancellation, and flush behavior. A cdpkit routing, decoder, lifecycle patch, or fork would invalidate a future selection and trigger the documented fallback rules.
...
- Historical v1 evidence identified exact cdpkit 0.4.0 as the candidate behind a replaceable adapter; current qualification remains pending strict v2 cross-platform requalification. Its named-event-params and unbounded-subscriber limitations remain explicit, and any selected client must pass the required-domain compatibility probe at runtime.
