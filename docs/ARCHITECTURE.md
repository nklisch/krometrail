...
For a temporal bundle, concise projection selects one primary retained artifact and reports exact outcome/resource omissions; expanded publishes compact handles for all generated outcomes; full adds bounded inline manifests. Every tier bounds identifier enumeration — resolved-range frame/event id vectors and manifest id vectors are presented as capped head slices with exact omitted counts — while the domain structures stay exact and complete sets remain reachable through the range handle, paginated listings, and canonical manifest resources. Visual artifact and filmstrip mappings asynchronously reuse the retained-artifact read boundary, validate identity, media type, length, and hash, and inline at most the selected primary image. Read failure degrades the response without discarding its structured result or canonical resource.
