...
krometrail debug launch "node index.js" --break index.js:42

# Debug TypeScript (ts-node)
krometrail debug launch "npx ts-node src/app.ts" --break src/app.ts:30

# Debug jest tests
krometrail debug launch "npx jest tests/order.test.js --no-coverage" \
	--break src/order.js:147

# Debug mocha tests
krometrail debug launch "npx mocha tests/**/*.test.js" --break src/api.js:83
```
...
```bash
krometrail debug break "src/order.ts:147 when discount < 0"
```
...
```bash
krometrail debug break "order.js:147 when discount < 0"
krometrail debug break "loop.js:25 when i === 99"
krometrail debug break "api.js:30 when req.method === 'POST'"
```
...
krometrail debug break --exceptions all       # including caught exceptions
```
...
```bash
krometrail debug eval "await user.fetchProfile()"
```
