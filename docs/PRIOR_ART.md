# Agent Lens — Prior Art Reference

Analysis of existing MCP-over-DAP projects, their approaches, and key insights for Agent Lens.
...
8. **No one has solved the token problem.** Every project returns raw DAP state with no token awareness. This is the gap Agent Lens exists to fill.
