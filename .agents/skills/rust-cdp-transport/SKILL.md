...
Evidence date: **2026-07-12**. The checked-in schema-v2 Linux/macOS qualification reports and generated decision are historical and intentionally unmodified: they predate the canonical-trace reconstructability contract and must not be treated as decisive until fresh evidence is produced. Decisive reports now require bounded sanitized canonical fixture, wire-observation, and runtime material from which validation recomputes every digest/count/result. The contract receives each frame, immediately acknowledges it, then performs bounded handoff; failed enqueue is an explicit capture gap after acknowledgement. It names observed `capture_elapsed_seconds` and `handoff_elapsed_seconds` measurements and measures acknowledgement from returned frame to ack completion only. Selection remains behind the replaceable adapter boundary; production lifecycle and capture implementation remain later work.
